...
//
// Changes by Swapscanner:
// - Modified to support higher solidity version.
...
contract CnStakingContract {
    ...
    event ApproveStakingWithdrawal(
        uint256 approvedWithdrawalId,
        address to,
        uint256 value,
        uint256 withdrawableFrom
    );
    event CancelApprovedStakingWithdrawal(
        uint256 approvedWithdrawalId,
        address to,
        uint256 value
    );
    ...
    //AddressBook event
    event ReviseRewardAddress(
        address cnNodeId,
        address prevRewardAddress,
        address curRewardAddress
    );
    ...
    address public constant ADDRESS_BOOK_ADDRESS =
    ...
    modifier notConfirmedRequest(uint256 _id) {
        require(
            requestMap[_id].state == RequestState.NotConfirmed,
            "Must be at not-confirmed state."
        );
        _;
    }
    ...
    {
        require(_contractValidator != address(0), "Validator is null.");
        isAdmin[_contractValidator] = true;
        for (uint256 i = 0; i < _cnAdminlist.length; i++) {
            require(
                !isAdmin[_cnAdminlist[i]] && _cnAdminlist[i] != address(0),
        ...
        require(
            _unlockTime.length != 0 &&
                _unlockAmount.length != 0 &&
                _unlockTime.length == _unlockAmount.length,
        ...
        for (uint256 i = 0; i < _unlockAmount.length; i++) {
            require(
                ...
                "Unlock time is not in ascending order."
            );
    ...
    function reviewInitialConditions()
        external
        onlyAdmin(msg.sender)
        beforeInit
    {
        require(
            lockupConditions.reviewedAdmin[msg.sender] == false,
    ...
    function depositLockupStakingAndInit() external payable beforeInit {
        uint256 requiredStakingAmount;
        uint256 cnt = lockupConditions.unlockAmount.length;
        for (uint256 i = 0; i < cnt; i++) {
            requiredStakingAmount = requiredStakingAmount.add(
                lockupConditions.unlockAmount[i]
            );
        }
        require(
            lockupConditions.allReviewed == true,
        ...
        validRequirement(adminList.length.add(1), requirement)
    {
        ...
        submitRequest(
            id,
            Functions.AddAdmin,
            bytes32(uint256(uint160(_admin)) << 96),
            0,
            0
        );
        confirmRequest(
    ...
    function submitClearRequest() external afterInit onlyAdmin(msg.sender) {
        uint256 id = requestCount;
    ...
    function submitWithdrawLockupStaking(
        address _to,
        uint256 _value
    ) external afterInit notNull(_to) onlyAdmin(msg.sender) {
        uint256 withdrawableStakingAmount;
        (, , , , withdrawableStakingAmount) = getLockupStakingInfo();
        require(
            _value > 0 && _value <= withdrawableStakingAmount,
            "Invalid value."
        );
        ...
        submitRequest(
            id,
            Functions.WithdrawLockupStaking,
            bytes32(uint256(uint160(_to)) << 96),
            bytes32(_value),
            0
        );
        confirmRequest(
    ...
    function submitApproveStakingWithdrawal(
        ...
        submitRequest(
            id,
            Functions.ApproveStakingWithdrawal,
            bytes32(uint256(uint160(_to)) << 96),
            bytes32(_value),
            0
        );
    ...
    function submitCancelApprovedStakingWithdrawal(
        uint256 _approvedWithdrawalId
    ) external afterInit onlyAdmin(msg.sender) {
        require(
            withdrawalRequestMap[_approvedWithdrawalId].to != address(0),
            "Withdrawal request does not exist."
        );
        require(
            withdrawalRequestMap[_approvedWithdrawalId].state ==
                WithdrawalStakingState.Unknown,
            "Invalid state."
        );
        ...
        submitRequest(
            id,
            Functions.CancelApprovedStakingWithdrawal,
            bytes32(_approvedWithdrawalId),
            0,
            0
        );
        confirmRequest(
    ...
    function submitUpdateRewardAddress(
        address _rewardAddress
    ) external afterInit notNull(_rewardAddress) onlyAdmin(msg.sender) {
        uint256 id = requestCount;
        submitRequest(
            id,
            Functions.UpdateRewardAddress,
            bytes32(uint256(uint160(_rewardAddress)) << 96),
            0,
            0
        );
        confirmRequest(
    ...
    function confirmRequest(
        ...
        bytes32 _secondArg,
        bytes32 _thirdArg
    ) public notConfirmedRequest(_id) onlyAdmin(msg.sender) {
    ...
    function revokeConfirmation(
        ...
        bytes32 _secondArg,
        bytes32 _thirdArg
    ) external notConfirmedRequest(_id) onlyAdmin(msg.sender) {
        bool hasConfirmed = false;
        uint256 confirmersCnt = requestMap[_id].confirmers.length;
        for (uint256 i = 0; i < confirmersCnt; i++) {
            if (msg.sender == requestMap[_id].confirmers[i]) {
    ...
    function withdrawLockupStaking(
        ...
        (, , , , withdrawableStakingAmount) = getLockupStakingInfo();
        require(
            withdrawableStakingAmount >= _value,
            "Value is not withdrawable."
        );
    ...
    function cancelApprovedStakingWithdrawal(
        uint256 _approvedWithdrawalId
    ) external onlyMultisigTx {
        require(
            withdrawalRequestMap[_approvedWithdrawalId].to != address(0),
        ...
        withdrawalRequestMap[_approvedWithdrawalId]
            .state = WithdrawalStakingState.Canceled;
        emit CancelApprovedStakingWithdrawal(
            _approvedWithdrawalId,
            withdrawalRequestMap[_approvedWithdrawalId].to,
            withdrawalRequestMap[_approvedWithdrawalId].value
        );
    }
    ...
    function updateRewardAddress(
        ...
        rewardAddress = _rewardAddress;
        AddressBookInterface(ADDRESS_BOOK_ADDRESS).reviseRewardAddress(
            _rewardAddress
        );
     ...
     */
    /// @notice stake KLAY
    function stakeKlay() external payable afterInit {
    ...
    receive() external payable afterInit {
        require(msg.value > 0, "Invalid amount.");
        staking = staking.add(msg.value);
        emit StakeKlay(msg.sender, msg.value);
    }
    ...
    function withdrawApprovedStaking(
        uint256 _approvedWithdrawalId
        ...
        require(
            withdrawalRequestMap[_approvedWithdrawalId].value <= staking,
            "Value is not withdrawable."
        );
        require(
            block.timestamp >=
                withdrawalRequestMap[_approvedWithdrawalId].withdrawableFrom,
        ...
        if (
            ...
            withdrawalRequestMap[_approvedWithdrawalId]
                .state = WithdrawalStakingState.Transferred;
            withdrawalRequestMap[_approvedWithdrawalId].to.transfer(
                withdrawalRequestMap[_approvedWithdrawalId].value
            );
            emit WithdrawApprovedStaking(
                _approvedWithdrawalId,
                withdrawalRequestMap[_approvedWithdrawalId].to,
                withdrawalRequestMap[_approvedWithdrawalId].value
            );
        ...
        for (uint256 i = 0; i < confirmersCnt; i++) {
            if (_admin == requestMap[_id].confirmers[i]) {
                if (i != confirmersCnt - 1) {
                    requestMap[_id].confirmers[i] = requestMap[_id].confirmers[
                        confirmersCnt - 1
                    ];
                }

                requestMap[_id].confirmers.pop();
                break;
            }
        }
    }
    ...
    function executeRequest(uint256 _id) private {
        ...
        if (requestMap[_id].functionId == Functions.AddAdmin) {
            ...
            (executed, ) = address(this).call(
                abi.encode(
                    0x70480275,
                    address(uint160(uint256(requestMap[_id].firstArg)))
                )
            );
        }
        ...
        if (requestMap[_id].functionId == Functions.ClearRequest) {
            ...
            (executed, ) = address(this).call(abi.encode(0x4f97638f));
        }
        ...
        if (requestMap[_id].functionId == Functions.WithdrawLockupStaking) {
            ...
            (executed, ) = address(this).call(
                abi.encode(
                    0x505ebed4,
                    address(uint160(uint256(requestMap[_id].firstArg))),
                    uint256(requestMap[_id].secondArg)
                )
            );
        }
    ...
    {
        address[] memory reviewers = new address[](
        ...
        returns (uint256[] memory, uint256[] memory, uint256, uint256, uint256)
    {
        ...
        uint256 amountWithdrawn = initialLockupStaking.sub(
    ...
    function getApprovedStakingWithdrawalInfo(
        ...
        external
        view
        returns (address, uint256, uint256, WithdrawalStakingState)
    {
        return (
